...
public:
	...
	double sigmoid_prime_advanced(double z)
	{
		return (z*(1-z));
	}
	...
	*/
	...
	void cost_derivative()
	{
			...
			for(int p =0; p<(net->sizes[n+1]); p++)
			{
				for(int i=0; i<batch_size;i++)
				{
					...
					for (int k =0; k<(net->sizes[n+2]); k++)
						temp += (net->derivatives[n+1][k][i]*net->weights[n][k][p]);
		...
		for (int n=0; n< (num_layers+1);n++ )
		{
			for(int p =0; p<(net->sizes[n+1]); p++)
			{
				for(int i=0; i<batch_size;i++)
				{
					net->delta_biases[n][p] = net->delta_biases[n][p] + net->derivatives[n][p][i];
				}
			}
		}
		...
		for (int n=0; n< (num_layers+1);n++ )
		{
			for(int p =0; p<(net->sizes[n+1]); p++)
			{
				...
				{	
					for(int i=0; i<batch_size;i++)
					{
						net->delta_weights[n][p][q]= net->delta_weights[n][p][q] + net->derivatives[n][p][i]*train_activations[n][q][i];
					}
				}
			}
		}
	...
	void update_mini_batch()
	{
		...
		for (int i=0; i<=num_layers; i++)
		{
			for(int j=0; j<net->sizes[i+1]; j++)
			{
				net->biases[i][j]  = net->biases[i][j] +  learning_rate * net->delta_biases[i][j]/test_set_size;
			}
		}
		...
		for (int i=0;i<=num_layers;i++)
		{
			for (int j = 0; j < net->sizes[i+1]; j++)
			{
				for(int k =0; k< net->sizes[i]; k++)
				{
					net->weights[i][j][k] += learning_rate*net->weights[i][j][k]/test_set_size;
				}
			}
		}
	}
	...
	void SGD(int epochs, int mini_batch_size, double eta)
	{
		...
		for (int i=0; i<epochs; i++)
		{
			...
			for (int j = 0; j< train_set_size; j+=batch_size)
			{
				...
				{
					r = randomising_array.at(j*batch_size + k);
